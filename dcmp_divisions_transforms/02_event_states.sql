CREATE TABLE IF NOT EXISTS event_states AS (
    ...
    award_points AS (
        SELECT
            event_key,
            SUM(award_points) AS sum_award_points
        FROM event_points
    ...
    SELECT
        ...
        CASE
            ...
            WHEN ANY_VALUE(award_state.award_count) <= 10 AND ANY_VALUE(award_points.sum_award_points) < 180 THEN 'Awards' -- checks that the award points were actually given out on TBA (sometimes there's a delay)
    ...
    LEFT JOIN matches ON events.event_key = matches.event_key
    WHERE events.event_type IN ('District Championship Division')
    GROUP BY
        events.event_key
)
