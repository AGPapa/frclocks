CREATE TABLE IF NOT EXISTS adjusted_district_rankings AS (
    ...
    events_remaining AS (
        SELECT
            ...
            SUM(CASE WHEN event_states.event_state IN ('Pre-Event', 'Qualifications') THEN 1 ELSE 0 END) AS incomplete_quals
    ...
    SELECT
        ...
        CAST(COALESCE(filtered_event_points.points, 0) + district_rankings.rookie_bonus + 4 * COALESCE(events_remaining.incomplete_quals, 0) AS INTEGER) AS inflated_points,
