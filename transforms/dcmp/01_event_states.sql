CREATE TABLE IF NOT EXISTS event_states AS (
    ...
    SELECT
        ...
        CASE
            ...
            WHEN ANY_VALUE(award_state.award_count) <= 10 OR ANY_VALUE(award_points.max_award_points) < 30 THEN 'Awards' -- checks that the award points were actually given out on TBA (sometimes there's a delay)
            WHEN ANY_VALUE(award_state.award_count) > 10 AND ANY_VALUE(award_points.max_award_points) = 30 THEN 'Completed'
            ELSE 'ERROR'
        END AS event_state,
