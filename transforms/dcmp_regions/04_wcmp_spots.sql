CREATE TABLE IF NOT EXISTS wcmp_spots AS (
    ...
    bonus_point_spots AS (
        SELECT
            ...
            CASE WHEN MAX(event_states.event_state) = 'Completed' AND MIN(event_states.event_state) = 'Completed'
    ...
    SELECT
        ...
        ANY_VALUE(district_lookup.wcmp_capacity)
            ...
            + SUM(COALESCE(bonus_point_spots.num_missing_qualifying_awards, 0)) -- If RAS was not awarded at a completed DCMP event, then an extra point spot is available
        AS wcmp_spots
