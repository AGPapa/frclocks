CREATE TABLE IF NOT EXISTS wcmp_spots_v2 AS (
    ...
    guaranteed_extra_spots_per_event AS (
        SELECT
            district_key,
            event_key,
            MIN(num_locked_teams) AS guaranteed_extra_spots
        FROM locked_teams_per_alliance
        GROUP BY 1, 2
    )
    SELECT
        wcmp_spots.district_key,
        ANY_VALUE(wcmp_spots.wcmp_spots)
            + COALESCE(SUM(guaranteed_extra_spots_per_event.guaranteed_extra_spots), 0) -- each live DCMP event guarantees one new winner alliance; sum guarantees across events
        AS wcmp_spots
    FROM wcmp_spots
    LEFT JOIN guaranteed_extra_spots_per_event ON wcmp_spots.district_key = guaranteed_extra_spots_per_event.district_key
    GROUP BY 1
)