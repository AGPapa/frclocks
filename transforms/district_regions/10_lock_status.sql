CREATE TABLE IF NOT EXISTS lock_status AS (
    ...
    SELECT
        ...
        ANY_VALUE(adjusted_district_rankings.region) AS region,
        ...
        WHEN ANY_VALUE(district_rankings_without_impact.events_remaining) = 0 AND ANY_VALUE(following_teams.following_team_key) IS NULL AND ANY_VALUE(adjusted_district_rankings.rank) > ANY_VALUE(district_lookup.dcmp_capacity) THEN '-'
    ...
    JOIN district_points_remaining ON adjusted_district_rankings.district_key = district_points_remaining.district_key AND adjusted_district_rankings.region = district_points_remaining.region
    JOIN district_lookup ON adjusted_district_rankings.district_key = district_lookup.district_key
    JOIN num_of_teams ON adjusted_district_rankings.region = num_of_teams.region
